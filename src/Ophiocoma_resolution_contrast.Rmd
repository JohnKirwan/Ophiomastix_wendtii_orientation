...
lapse      <- runif(N,0,.65)
threshold  <- rnorm(N,-.5,0.5)    # Threshold must be > 0 and can be > 1.
width      <- rnorm(N,-.5,0.5)    # Width must be > 0 and can be > 1.
...
res.empty.formula <- bf(success ~ 
                      base + 
                      (1 - base - lapse) * 
                      inv_logit( 4.39*(	1 - exp(threshold)	)	/  exp(width)  ),
...
res.psych.priors  <- c(
         ...
         prior(normal(-.5,.5),         nlpar= 'threshold',  class = 'b'   ),
	       prior(normal(-.5,.5),         nlpar = 'width',     class = 'b'     ))
...
### Version with H&B formulation with estimated lapse and base
...
```{r}
loo_compare(res.psych.fit,res.empty.fit)
```
...
res.smpl <- posterior_samples(res.psych.fit)
```


```{r}
post_sum <- posterior_summary(res.psych.fit, 
                  pars=c(
...
Plot the threshold and width tranformed.

```{r}
param_trans <- function(x){
  out = exp(x)*max(res$target)
  return(out)}
...
```{r}
  pp_check(res.psych.fit,nsamples=400, type="bars",) + theme_classic() + theme(legend.position='none') 
```


```{r}
  pp_check(res.psych.fit, type = "rootogram", nsamples = 100, style = "standing", prob=0.9) +   theme_classic()  
```
...
turn.idx <- vector(length = length(res.smpl[,1]))
x <- seq(from=0,to=1.5,length.out = N)
newdat <- data.frame(T = T)

plot( NULL, xlim = c(0,1.5), ylim = c(0.1,.7) , 
      xlab = "Scaled stimulus strength", ylab = "Proportion correct response" )
abline (h=1/6, lty=2)
i = 1
for ( i  in 1:length(res.smpl[,1])){

  vals[i,] <- res.smpl$b_base_Intercept[i] + 
          (1 - res.smpl$b_base_Intercept[i] - res.smpl$b_lapse_Intercept[i]) * 
          plogis(4.39*(	x - exp(res.smpl$b_threshold_Intercept[i])	)	/ 
                    exp(res.smpl$b_width_Intercept[i]) )
...
hist(turning.pts,breaks = 200)
```

Below, are two confidence intervals for the turning point.

```{r}
PI(turning.pts,prob=.95)
```

```{r}
HPDI(turning.pts,prob=.95)
```
...
```{r}
HPDI(exp(res.smpl$b_threshold_Intercept)*max(res$target))
```
...
base       <- rnorm(N,0.167,0.05)  # Base is close to random probability 
lapse      <- rbeta(N,3,5)       # Lapse is wide range - should not overlap base rate
threshold  <- rnorm(N,-.5,.5)    # Threshold must be > 0 and can be > 1.
width      <- rnorm(N,-.5,.5)    # Width must be > 0 and can be > 1.
                                   # Varying intercept lapse
                                   # Varying intercepts base
...
con.psych.formula <- bf(success ~ 
                      base + 
                      (1 - base - lapse) * 
                      inv_logit( 4.39*(	contrast - exp(threshold)	)	/  exp(width)  ),
...
con.empty.formula <- bf(success ~ 
                      ...
                      inv_logit(4.39*(	1 - exp(threshold)	)	/
                      ( exp (width) ) ),
  base      ~ 1, #+ #(1|individual), # guess rate has a single value
	lapse     ~ 1, #+ #(1|individual), # lapse rate can be different for different chicks
	threshold ~ 1, #+ #(1|individual), # threshold changes with all effects
...
con.empty.fit  <- brm(con.empty.formula, data = con, family = bernoulli("identity"), iter = 5000,
              chains = 4, prior = con.psych.priors, control = list(adapt_delta=0.9999))
